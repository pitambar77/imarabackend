...
export const createZanzibardetails = async (req, res) => {
  try {
    ...
    /* ---------- CREATE ---------- */
    const zanzibar = await Zanzibar.create({
      ...formData,
      image: mainImage,
    ...
    res.status(201).json({
      message: "Zanzibar details created successfully",
      data: zanzibar,
    });
...
export const getAllZanzibardetails = async (req, res) => {
  try {
    const data = await Zanzibar.find().sort({ createdAt: -1 });
    res.json(data);
  } catch (err) {
    res.status(500).json({ message: err.message });
  }
...
export const getZanzibardetailsById = async (req, res) => {
  try {
    const data = await Zanzibar.findById(req.params.id);
...
export const updateZanzibardetails = async (req, res) => {
  try {
    const { id } = req.params;
    ...
    const updated = await Zanzibar.findByIdAndUpdate(id, updateData, {
      new: true,
      runValidators: true,
    });

    res.json({ message: "Zanzibar updated successfully", data: updated });
...
export const deleteZanzibardetails = async (req, res) => {
  try {
    const data = await Zanzibar.findById(req.params.id);
    if (!data) return res.status(404).json({ message: "Not found" });
    ...
    await Zanzibar.findByIdAndDelete(req.params.id);

    res.json({ message: "Zanzibar deleted successfully" });
