import Termscondition from "../../models/TermsCondition/Termscondition.js";
...
export const createTermscondition = async (req, res) => {
  try {
    ...
    const newDoc = await Termscondition.create({
      ...formDataParsed,
...
export const getAllTermscondition = async (req, res) => {
  try {
    const data = await Termscondition.find().sort({
      createdAt: -1,
    });
...
export const getTermsconditionById = async (req, res) => {
  try {
    const page = await Termscondition.findById(req.params.id);
...
export const updateTermscondition = async (req, res) => {
  try {
    ...
    const updated = await Termscondition.findByIdAndUpdate(id, updateData, {
      new: true,
      runValidators: true,
    });
...
export const deleteTermscondition = async (req, res) => {
  try {
    const doc = await Termscondition.findById(req.params.id);

    if (!doc) {
      return res.status(404).json({
        message: "Termscondition page not found",
      });
    }
