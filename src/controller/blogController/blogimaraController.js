import Blogimara from "../../models/ImaraBlog/Blogimara.js";
...
export const createBlog = async (req, res) => {
  try {
    ...
    const existing = await Blogimara.findOne({ slug: body.slug });
    if (existing) {
    ...
    const blog = await Blogimara.create(body);
...
export const getBlogs = async (req, res) => {
  try {
    const blogs = await Blogimara.find()
      .sort({ createdAt: -1 })
...
export const getBlogBySlug = async (req, res) => {
  try {
    const blog = await Blogimara.findOne({ slug: req.params.slug }).populate(
      "relatedBlogs",
    );
...
if (body.date) {
  body.date = new Date(body.date);
    ...
    const blog = await Blogimara.findByIdAndUpdate(id, body, {
      new: true,
      runValidators: true,
    });
...
export const deleteBlog = async (req, res) => {
  try {
    const blog = await Blogimara.findByIdAndDelete(req.params.id);
...
export const addSection = async (req, res) => {
  try {
    ...
    const blog = await Blogimara.findById(blogId);
