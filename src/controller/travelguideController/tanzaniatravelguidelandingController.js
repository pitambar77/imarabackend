...
import Tanzaniatravelguidelanding from "../../models/Travelguide/Tanzaniatravelguidelanding.js";
...
export const createTanzaniadestinationlanding = async (req, res) => {
  try {
    ...
    const newDoc = await Tanzaniatravelguidelanding.create({
      ...formDataParsed,
...
export const getAllTanzaniadestinationlanding = async (req, res) => {
  try {
    const data = await Tanzaniatravelguidelanding.find().sort({
      createdAt: -1,
    });
...
export const getTanzaniadestinationlandingById = async (req, res) => {
  try {
    const page = await Tanzaniatravelguidelanding.findById(req.params.id);
...
export const updateTanzaniadestinationlanding = async (req, res) => {
  try {
    ...
    const updated = await Tanzaniatravelguidelanding.findByIdAndUpdate(
      id,
      updateData,
      {
        new: true,
...
export const deleteTanzaniadestinationlanding = async (req, res) => {
  try {
    const doc = await Tanzaniatravelguidelanding.findById(req.params.id);
    ...
    await Tanzaniatravelguidelanding.findByIdAndDelete(req.params.id);
