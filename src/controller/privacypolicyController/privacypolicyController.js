import Privacypolicy from "../../models/PrivacyPolicy/Privacypolicy.js";
...
export const createPrivacypolicy = async (req, res) => {
  try {
    ...
    const newDoc = await Privacypolicy.create({
      ...formDataParsed,
...
export const getAllPrivacypolicy = async (req, res) => {
  try {
    const data = await Privacypolicy.find().sort({
      createdAt: -1,
    });
...
export const getPrivacypolicyById = async (req, res) => {
  try {
    const page = await Privacypolicy.findById(req.params.id);
...
export const updatePrivacypolicy = async (req, res) => {
  try {
    ...
    const updated = await Privacypolicy.findByIdAndUpdate(id, updateData, {
      new: true,
      runValidators: true,
    });
...
export const deletePrivacypolicy = async (req, res) => {
  try {
    const doc = await Privacypolicy.findById(req.params.id);
